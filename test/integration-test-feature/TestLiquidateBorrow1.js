...
contract('liquidate-1: disable liquidate public borrower', async (accounts) => {
    ...
    before(async () => {
        ...
        await sysConfig.SetPublicBorrower(ctx, accounts[1]);
        await borrow.simpleBorrow(ctx, dEther, accounts[1], web3.utils.toWei('8000'));
    });
    let reason = "disable liquidate public borrower";
    it('liquidate-1-1: liquidate 0 or -1', async () => {
        await liquidateBorrow.failLiquidateBorrow(ctx, ctx.dEther, accounts[0], accounts[1], ctx.USDC, 0, reason);
        await liquidateBorrow.failLiquidateBorrow(ctx, ctx.dEther, accounts[0], accounts[1], ctx.USDT, 0, reason);
        await liquidateBorrow.failLiquidateBorrow(ctx, ctx.dEther, accounts[0], accounts[1], ctx.CFSC, 0, reason);
        await liquidateBorrow.failLiquidateBorrow(ctx, ctx.dEther, accounts[0], accounts[1], ctx.USDC, maxUint256, reason);
        await liquidateBorrow.failLiquidateBorrow(ctx, ctx.dEther, accounts[0], accounts[1], ctx.USDT, maxUint256, reason);
        await liquidateBorrow.failLiquidateBorrow(ctx, ctx.dEther, accounts[0], accounts[1], ctx.CFSC, maxUint256, reason);
    });
    it('liquidate-1-2: liquidate some value', async () => {
        await liquidateBorrow.failLiquidateBorrow(ctx, ctx.dEther, accounts[0], accounts[1], ctx.USDC, 800, reason);
        await liquidateBorrow.failLiquidateBorrow(ctx, ctx.dEther, accounts[0], accounts[1], ctx.USDT, 800, reason);
        await liquidateBorrow.failLiquidateBorrow(ctx, ctx.dEther, accounts[0], accounts[1], ctx.CFSC, 800, reason);
    });
    it('liquidate-1-3: liquidate some value that exceed borrows', async () => {
        let repayAmount = web3.utils.toWei('8100');
        await liquidateBorrow.failLiquidateBorrow(ctx, ctx.dEther, accounts[0], accounts[1], ctx.USDC, repayAmount, reason);
        await liquidateBorrow.failLiquidateBorrow(ctx, ctx.dEther, accounts[0], accounts[1], ctx.USDT, repayAmount, reason);
        await liquidateBorrow.failLiquidateBorrow(ctx, ctx.dEther, accounts[0], accounts[1], ctx.CFSC, repayAmount, reason);
    });
});
